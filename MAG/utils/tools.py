...
def save_model(model, dataset=''):
    ...
    torch.save(model.state_dict(), f'pre_trained_models/best_model_MAG_{dataset}.pt')


def load_model(dataset=''):
    # name = save_load_name(args, name)
    with open(f'pre_trained_models/best_model_MAG_{dataset}.pt', 'rb') as f:
...
def save_hidden(tensor, dataset=''):
    ...
    torch.save(tensor, f'hidden_vectors/MAG_{dataset}.pt')


def load_hidden(dataset=''):
    with open(f'hidden_vectors/MAG_{dataset}.pt', 'rb') as f:
        buffer = io.BytesIO(f.read())
    H = torch.load(buffer)
    return H
